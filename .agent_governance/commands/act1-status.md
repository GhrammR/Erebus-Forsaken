...
* [x] One placeholder class (Myrmidon) drawn procedurally
  (`art/procedural/classes/myrmidon_sprite.tscn` — palette: bronze,
...
* [x] Pause menu opens / closes on Esc via PlayerInput.pause_pressed;
  Resume / Quit buttons working; `get_tree().paused` ownership lives
  in the pause scene
* [x] AD-02 single-player.tscn: `player.assign_class(ClassData)`
...
* [x] WASD input actions added to project.godot
  (move_up/down/left/right bound to W/A/S/D)
* [x] `--movement` cmdline flag in main.gd launches the workbench
* [x] combat-validator + scene-auditor: all checks PASS
* [~] Visual workbench playtest — awaiting user (godot -- --movement,
  or F6 on test/movement_workbench.tscn)
