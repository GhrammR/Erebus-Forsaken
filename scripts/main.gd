...
func _ready() -> void:
	...
	if "--verify12" in args:
		...
		return
	if "--verify13" in args:
		add_child(load("res://test/stage13_verify.tscn").instantiate())
		return
	if "--verify14" in args:
		add_child(load("res://test/stage14_verify.tscn").instantiate())
		return
	if "--verify15" in args:
		add_child(load("res://test/stage15_verify.tscn").instantiate())
		return
	if "--verify15_1" in args:
		add_child(load("res://test/stage15_1_verify.tscn").instantiate())
		return
	if "--verify16" in args:
		add_child(load("res://test/stage16_verify.tscn").instantiate())
		return
	if "--verify17_5" in args:
		add_child(load("res://test/stage17_5_verify.tscn").instantiate())
		return
	if "--render-sprites" in args or "--render_sprites" in args:
		add_child(load("res://test/sprite_render.tscn").instantiate())
		return
	if "--maw_diag" in args:
